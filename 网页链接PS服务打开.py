...
def send_to_ps():
    ...
    try:
        ...
        print("Opening temp image in Photoshop...")
        # 1. 临时以新文档形式打开刚刚下载的高清原图，获取它的真实尺寸
        temp_doc = psApp.Open(temp_path)
        
        # 如果刚才没有打开的文档，按照原图大小新建一个
        if not has_doc:
            print("No active document, creating a new one matching image size.")
            target_doc = psApp.Documents.Add(temp_doc.Width, temp_doc.Height, temp_doc.Resolution, "New Document")
            # 【修复】新建画板后，焦点会转移到新画板。必须把焦点切回临时图片，才能成功复制图层
            psApp.ActiveDocument = temp_doc
            
        print("Duplicating image layer to target document...")
        # 2. 获取原图层并直接跨文档复制
